...
struct Node {
    int r, c, dir, count;
    Node(int R, int C, int Dir, int Count) : r(R), c(C), dir(Dir), count(Count) {}
};
...
vector<vector<vector<int>>> checked; // 방향별로 체크

int dr[] = {1, 0, -1, 0};  // 하, 좌, 상, 우 (0, 1, 2, 3)
int dc[] = {0, -1, 0, 1};
        ...
        for (int i = 0; i < 3; i++) {
            ...
            switch (i) {
                case 0:
                    ...
                    break;
                case 1:
                    nextDir = (curDir + 1) % 4; // 시계 방향 회전
                    break;
                case 2:
                    nextDir = (curDir + 3) % 4; // 반시계 방향 회전
                    break;
            }
...
int main() {
    ...
    checked.resize(H, vector<vector<int>>(W, vector<int>(4, INF))); // 3차원 배열 초기화
